...
def parseGetBucketQuota(xml, headers=None):
    ...
    return GetBucketQuotaResponse(quota=common_util.toInt(quota))  
...
def parseListVersions(xml, headers=None):
    ...
    head = ObjectVersionHead(name=Name, prefix=Prefix, delimiter=Delimiter, keyMarker=KeyMarker, versionIdMarker=VersionIdMarker,
...
def __parseCommonHeader(headers, option):
    ...
    option.contentLength = common_util.toInt(headers.get('content-length'))
